1. Build Product Service Image
   docker build -t rupesh1997/payment-service:1.0.0 \
   -f ../docker/backend/payment-service/Dockerfile .

2. Run Product Service Container
   docker run -d -p 8084:8084 \
   --name payment-service \
   --network=purely-goods-nw \
   -e SPRING_DATASOURCE_URL=jdbc:mysql://product-db:3306/products \
   -e SPRING_DATASOURCE_USERNAME=root \
   -e SPRING_DATASOURCE_PASSWORD=root \
   rupesh1997/payment-service:1.0.0
...
7. Check Product Service Health Endpoint
    - http://localhost:8084/api/v1.0.0/actuator/health
